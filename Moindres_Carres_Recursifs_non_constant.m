...
function [MCR1] = Moindres_Carres_Recursifs_non_constant(N,B,A,B0,A0,alpha,bruit) 
...
for i=nb1+1:N ;
    Y(i)=-B(1)*(1-exp(-i))*Y(i-1);
    for j=2:nb1;
        Y(i)=Y(i)-B(j)*(1+exp(-i))*Y(i-j);
    end
    for j=1:na1;
        Y(i)=Y(i)+A(j)*(1+exp(-i))*U(i-j);
    end 
end
if bruit==true
   b=randn(1,N);
...
%=========traçage des courbes=========== 
n=na+nb;
f3 = figure('Name','Moindres_Carres_Recursifs_non_constant');
for i=1:nb+na;
    subplot(na,nb,i)
